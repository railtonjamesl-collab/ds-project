...
Now that we have our sampled our test data, we can split the data for training and testing, which can be used for evaluation. We will consider a 70/30 split. This means that 70% of our sampled data is used for training and 30% for  
...
summary(glm.fit)
```
Here we see that all the covariates are majorly significant to predicting whether a transaction is fraud or not, with an AIC = 37213. The AIC figure is of use for future comparisons of other models.
